...
static const yytype_uint8 yyprhs[] =
{
      ...
      82,    84,    86,    88,    90,    92,    93,   103,   105,   106,
     109,   112,   113,   116,   119,   120
};
...
static const yytype_int8 yyrhs[] =
{
      ...
      12,    -1,    -1,    49,    27,    37,    58,    38,    41,    57,
      59,    42,    -1,    47,    -1,    -1,    60,    59,    -1,    61,
      59,    -1,    -1,    46,    30,    -1,    50,    30,    -1,    -1,
      41,    62,    59,    42,    -1
};
...
static const char *const yytname[] =
{
  ...
  "ASSGN", "EXPR", "E", "T", "F", "REL_OP", "MAIN", "$@1", "EMPTY_LISTVAR",
  "STMT", "STMT_NO_BLOCK", "BLOCK", "$@2", 0
};
#endif
...
#line 86 "parser.y"
    { 
                ...
                if (check_symbol_table(sym_table, (yyvsp[(1) - (3)]))) { 
                    char err_msg[100]; 
                    sprintf(err_msg, "Redeclaration of variable '%s'", (yyvsp[(1) - (3)])); 
                    yyerror(err_msg); 
                } else { 
                    // Create and insert the new symbol 
                    int type = get_symbol_type(current_type); 
                    int size = get_type_size(current_type); 
                    symbol* sym = init_symbol((yyvsp[(1) - (3)]), size, type, yylineno, current_scope); 
                    insert_into_table(sym_table, sym); 
                    insert_value_to_name(sym_table, (yyvsp[(1) - (3)]), (yyvsp[(3) - (3)])); 
                } 
            }
    break;
...
#line 101 "parser.y"
    { 
                // Check if symbol is in the table 
                if (check_symbol_table(sym_table, (yyvsp[(1) - (1)]))) { 
                    char err_msg[100]; 
                    sprintf(err_msg, "Redeclaration of variable '%s'", (yyvsp[(1) - (1)])); 
                    yyerror(err_msg); 
                } else { 
                    // Create and insert the new symbol (without value) 
                    int type = get_symbol_type(current_type); 
                    int size = get_type_size(current_type); 
                    symbol* sym = init_symbol((yyvsp[(1) - (1)]), size, type, yylineno, current_scope); 
                    insert_into_table(sym_table, sym); 
                } 
            }
    break;
...
#line 124 "parser.y"
    { 
            ...
            if (!check_symbol_table(sym_table, (yyvsp[(1) - (3)]))) { 
                char err_msg[100]; 
                sprintf(err_msg, "Undeclared variable '%s'", (yyvsp[(1) - (3)])); 
                yyerror(err_msg); 
            } else { 
                insert_value_to_name(sym_table, (yyvsp[(1) - (3)]), (yyvsp[(3) - (3)])); 
            } 
...
/* Line 1455 of yacc.c  */
#line 136 "parser.y"
    { (yyval) = (yyvsp[(1) - (3)]); /* Relational operations not implemented in this version */ }
    break;
...
#line 141 "parser.y"
    {  
            (yyval) = perform_arithmetic((yyvsp[(1) - (3)]), "+", (yyvsp[(3) - (3)])); 
        }
    break;
...
#line 161 "parser.y"
    { 
            ...
            if (!check_symbol_table(sym_table, (yyvsp[(1) - (1)]))) { 
                char err_msg[100]; 
                sprintf(err_msg, "Undeclared variable '%s'", (yyvsp[(1) - (1)])); 
                yyerror(err_msg); 
                (yyval) = strdup("0"); 
            } else { 
                ...
                while (current != NULL) { 
                    if (strcmp(current->name, (yyvsp[(1) - (1)])) == 0) { 
                        if (current->value != NULL) { 
                            (yyval) = strdup(current->value); 
                        } else { 
                            char err_msg[100]; 
                            sprintf(err_msg, "Uninitialized variable '%s'", (yyvsp[(1) - (1)])); 
                            yyerror(err_msg); 
                            (yyval) = strdup("0"); 
                        } 
                        break; 
